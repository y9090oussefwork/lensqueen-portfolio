                            ...
                            @foreach($languages as $key => $language)
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="lang-tab-{{ $key }}" role="tabpanel">
                                    <form method="post" action="{{ route('admin.template.update', [$section,$language->id]) }}" enctype="multipart/form-data">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="input-group ">
                                                            <input type="text" name="{{ $name }}[{{ $language->id }}]"
                                                                   ...
                                                                   value="{{ old($name, isset($templateMedia->description->{$name}) ? $templateMedia->description->{$name} : '') }}">
                                                            <div class="input-group-append">
                                                                <button class="btn btn-outline-primary iconPicker"
                                                                        data-icon="{{ old($name, isset($templateMedia->description->{$name}) ? @$templateMedia->description->{$name} : '') }}"
                                        ...
                                        <button type="submit" class="btn waves-effect waves-light btn-rounded btn-primary btn-block mt-3 {{($i == 0)? 'd-none':''}}"> @lang('Save Change')</button>
                                    </form>
                                </div>
                            @endforeach
                        </div>

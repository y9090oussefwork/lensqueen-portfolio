                    ...
                    @forelse($tickets as $key => $ticket)
                        <tr>
                            ...
                            <td data-label="Submitted By">
                                @if($ticket->user_id)
                                    <a href="{{ route('admin.user-edit', $ticket->user_id)}}">
                                        <div class="d-flex no-block align-items-center">
                                            <div class="mr-3"><img src="{{ getFile(config('location.user.path').optional($ticket->user)->image)}}" alt="user" class="rounded-circle" width="45" height="45"></div>
                                            <div class="">
                                                <h5 class="text-dark mb-0 font-16 font-weight-medium">{{optional($ticket->user)->fullname}}</h5>
                                                <span class="text-muted font-14"><span>@</span>{{optional($ticket->user)->username}}</span>
                                            </div>
                                        </div>
                                ...
                                @else
                                    <div class="d-flex no-block align-items-center">
                                        <div class="mr-3"><img src="{{ getFile(config('location.user.path'))}}" alt="user" class="rounded-circle" width="45" height="45"></div>
                                        <div class="">
                                            <h5 class="text-dark mb-0 font-16 font-weight-medium">{{$ticket->name}}</h5>
                                        </div>
                                    </div>
                                @endif
                            </td>

...
@push('script')
    ...
    <script>
        ...
        $(document).ready(function () {
            var id, minAmount, maxAmount, baseSymbol, fixCharge, percentCharge,name, currency, amount, gateway, pro_id;
            $(document).on('click', '.addFund', function () {
                ...
                $('.method-name').text(`@lang('Payment By') ${name} - ${currency}`);
            ...
            $(".checkCalc").on('click', function () {
                ...
                $('#loading').show();
                // Notiflix.Block.pulse('.notiflix-loader');
                ...
                $.ajax({
                    url: "{{route('user.purchase.product.request')}}",
                    type: 'POST',
                    data: {
                        amount,
                        gateway,

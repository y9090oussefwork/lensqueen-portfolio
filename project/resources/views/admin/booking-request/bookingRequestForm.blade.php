...
@section('title')
    @lang('User Booking Request Form')
@endsection
    ...
    <div class="row">
        <div class="col-12">
            <div class="card shadow">
                <div class="card-body">
                    <h4 class="card-title mb-3"> @lang('Additional Information For Booking Request')</h4>
                    <form action="{{route('admin.booking.request.form.update', $id)}}" method="post" enctype="multipart/form-data" class="form-row text-left preview-form">
                        @csrf
                        @method('put')
                        @if($bookingRequestInfo->booking_info)
                            @foreach($bookingRequestInfo->booking_info as $k => $v)
                        ...
                        @else
                            <div class="col-md-12">
                                <p class="text-center d-block text-danger text-28 p-2">@lang("Booking Request Form Not Submitted by User.")</p>
                            </div>
                        @endif

                        @if($bookingRequestInfo->status !=0)
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="text-16"><strong>@lang('Note')</strong></label>
                                        <textarea name="note" cols="10" rows="2" class="form-control form-control-lg">{{$bookingRequestInfo->note}}</textarea>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                @if($bookingRequestInfo->booking_info || $bookingRequestInfo->note)
